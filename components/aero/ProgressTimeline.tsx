...
export default function ProgressTimeline() {
  ...
  const milestoneCard = (milestone: (typeof MILESTONES)[number]) => {
    ...
    return (
      <div
        key={milestone.id}
        className={`${prefersReduced ? "w-[260px] md:w-[300px]" : "flex-shrink-0 w-[260px] md:w-[300px]"} ${isMilestoneFuture ? "opacity-40" : ""}`}
      >
        {/* Card */}
        <div
          className={`
    ...
    return (
      <>
        <section className="relative py-12">
          ...
          {/* Timeline line — fully drawn */}
          <div className="relative mx-8 mb-8">
            <div className="h-[2px] bg-[#e3a72f]/10 w-full rounded-full" />
            <div className="absolute top-0 left-0 h-[2px] bg-[#e3a72f] rounded-full w-full" />
          </div>

          {/* Vertical wrapping grid */}
          <div className="flex flex-wrap gap-6 md:gap-8 px-8 justify-center">
            {MILESTONES.map((milestone) => milestoneCard(milestone))}
          </div>
        </section>
