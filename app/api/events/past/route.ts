...
import { isPastEvent, normalizeEvent, sortEventsByDateAscending } from "@/lib/events"
...
export async function GET() {
  try {
    ...
    const events = sortEventsByDateAscending(data.map((event) => normalizeEvent(event as Record<string, unknown>))).filter(isPastEvent)
